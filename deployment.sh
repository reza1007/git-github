...
OLD_VERSION=$(aws ec2 describe-launch-template-versions --launch-template-name $INSTANCE_TEMPLATE_NAME --query 'LaunchTemplateVersions[*].[VersionNumber]' --output text  |head -n 1)

# Create a new version of the ASG template
aws ec2 create-launch-template-version --launch-template-name $INSTANCE_TEMPLATE_NAME --version-description $TEMPLATE_VERSION_DESCRIPTION --source-version $OLD_VERSION --launch-template-data '{"ImageId":"'$AMI_ID'"}'
